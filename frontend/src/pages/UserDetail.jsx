...
const UserDetail = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get("http://localhost:8000/users/user-details", {
          withCredentials: true, // This is equivalent to `credentials: 'include'` in `fetch`
        });

        // Assuming the response contains user details directly
        setUserDetails(response.data);
      } catch (err) {
        setError(err.message); // Handle error in fetching user details
      } finally {
        setLoading(false); // Loading is false when the request finishes (success or error)
      }
    };

    const fetchQuizResults = async () => {
      try {
        const response = await axios.get("http://localhost:8000/quizresult", {
          withCredentials: true, // Same as above
        });

        // Ensure quizResults is always an array
        setQuizResults(response.data.results || []); // Default to empty array if no results
      } catch (err) {
        setError(err.message); // Handle error in fetching quiz results
      }
    };
